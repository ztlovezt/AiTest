          ...
          <template v-for="config in configs" :key="config?.id || 'unknown'">
            <div v-if="config && config.id" class="config-card">
              ...
              <div class="config-details">
                ...
                <div v-if="config.provider === 'ppocr'" class="detail-item">
                  <label>{{ $t('configuration.ocr.useGpu') }}:</label>
                  <span>{{ config.use_gpu ? $t('configuration.common.yes') : $t('configuration.common.no') }}</span>
                </div>
                <div class="detail-item">
          ...
          <form @submit.prevent="saveConfig">
            ...
            <div class="form-group">
              ...
              <select
                ...
                <option value="ppocr">{{ $t('configuration.ocr.providers.ppocr') }}</option>
            ...
            <div v-if="isOnlineProvider" class="form-group">
              <label>{{ $t('configuration.ocr.apiKey') }} <span class="required">*</span></label>
              <input
                v-model="configForm.api_key"
                type="password"
                class="form-input"
                :placeholder="$t('configuration.ocr.apiKeyPlaceholder')"
                :required="isOnlineProvider && !isEditing">
            </div>

            <div v-if="isOnlineProvider" class="form-group">
              <label>{{ $t('configuration.ocr.baseUrl') }} <span class="required">*</span></label>
              <input
                v-model="configForm.base_url"
                type="url"
                class="form-input"
                :placeholder="$t('configuration.ocr.baseUrlPlaceholder')"
                :required="isOnlineProvider">
            ...
            <div v-if="configForm.provider === 'ppocr'" class="form-group">
              <label class="checkbox-label">
                <input v-model="configForm.use_gpu" type="checkbox">
                <span class="checkmark"></span>
                {{ $t('configuration.ocr.useGpu') }}
              </label>
            </div>
