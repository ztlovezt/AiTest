<template>
  <div class="device-management">
    ...
    >
      <el-table-column
        prop="name"
        :label="t('appAutomation.device.deviceName')"
        min-width="150"
      >
      ...
      <el-table-column
        prop="device_id"
        :label="t('appAutomation.device.deviceId')"
        min-width="180"
      />

      <el-table-column
        prop="status"
        :label="t('appAutomation.device.status')"
        width="100"
      >
        <template #default="{ row }">
      ...
      <el-table-column
        prop="locked_at"
        :label="t('appAutomation.device.lastUsed')"
        width="180"
      >
        <template #default="{ row }">
      ...
      <el-table-column
        ...
        :label="t('appAutomation.device.platformVersion')"
        width="120"
      />

      <el-table-column
        prop="connection_type"
        :label="t('appAutomation.device.platform')"
        width="120"
      >
        <template #default="{ row }">
          <el-tag
      ...
      <el-table-column
        prop="ip_address"
        :label="t('appAutomation.common.ipAddress')"
        width="150"
      >
        <template #default="{ row }">
      ...
      <el-table-column
        prop="usage_count"
        :label="t('appAutomation.device.usageCount')"
        width="100"
      />

      <el-table-column
        prop="updated_at"
        :label="t('appAutomation.device.createTime')"
        width="180"
      >
        <template #default="{ row }">
      ...
      <el-table-column
        :label="t('appAutomation.common.operation')"
        width="300"
        fixed="right"
      >
        <template #default="{ row }">
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="addRemoteDialogVisible = false">{{
    ...
    >
      <el-descriptions v-if="selectedDevice" :column="2" border>
        ...
        <el-descriptions-item :label="t('appAutomation.device.platform')">
          <el-tag
            :type="
              getConnectionType(selectedDevice.connection_type) === 'local'
                ? 'primary'
                : 'warning'
            "
            size="small"
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="deviceInfoDialogVisible = false">{{
...
const emptyText = computed(() => t("appAutomation.messages.noDevicesHint"));
...
const remoteDeviceRules = {
  ip_address: [
    {
      required: true,
      message: t("appAutomation.messages.pleaseEnterIpAddress"),
      trigger: "blur",
    },
    {
  ...
  port: [
    {
      required: true,
      message: t("appAutomation.messages.pleaseEnterPort"),
      trigger: "blur",
    },
  ],
};
...
const getDevices = async () => {
  ...
  try {
    ...
    ElMessage.error(
      t("appAutomation.messages.getDeviceListFailed") +
        ": " +
        (error.message || t("appAutomation.messages.unknownError")),
    );
  } finally {
    loading.value = false;
  }
...
const refreshDevices = async () => {
  ...
  try {
    ...
    if (res.data.success) {
      ...
      ElMessage.success(
        res.data.message || t("appAutomation.messages.deviceListRefreshed"),
      );
    } else {
      ElMessage.error(
        res.data.message || t("appAutomation.messages.refreshDeviceListFailed"),
      );
    }
  } catch (error) {
    console.error("鍒锋柊璁惧鍒楄〃澶辫触:", error);
    ElMessage.error(
      t("appAutomation.messages.refreshDeviceListFailed") +
        ": " +
        (error.message || t("appAutomation.messages.unknownError")),
    );
  } finally {
...
const connectRemoteDevice = async () => {
  ...
  remoteDeviceFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (res.data.success) {
        ElMessage.success(
          res.data.message || t("appAutomation.messages.remoteDeviceConnected"),
        );
        ...
        ElMessage.error(
          res.data.message ||
            t("appAutomation.messages.connectRemoteDeviceFailed"),
        );
      }
    } catch (error) {
...
const reconnectDevice = async (device) => {
  ...
  try {
    ...
    if (res.data.success) {
      ...
      ElMessage.error(
        res.data.message || t("appAutomation.messages.deviceReconnectFailed"),
      );
    }
  } catch (error) {
...
const disconnectDevice = async (device) => {
  try {
    await ElMessageBox.confirm(
      t("appAutomation.messages.disconnectDeviceConfirm", {
    ...
    if (res.data.success) {
      ...
      ElMessage.error(
        res.data.message || t("appAutomation.messages.disconnectDeviceFailed"),
      );
    }
  } catch (error) {
    if (error !== "cancel") {
      ...
      ElMessage.error(
        t("appAutomation.messages.disconnectDeviceFailed") +
          ": " +
          (error.message || t("appAutomation.messages.unknownError")),
      );
    }
...
const lockDevice = async (device) => {
  try {
    await ElMessageBox.confirm(
      t("appAutomation.messages.lockDeviceConfirm", {
        name: device.name || device.device_id,
      }),
      t("appAutomation.messages.tip"),
      {
    ...
    if (res.data.success) {
      ...
      ElMessage.error(
        res.data.message || t("appAutomation.messages.lockDeviceFailed"),
      );
    }
  } catch (error) {
    if (error !== "cancel") {
      ...
      ElMessage.error(
        t("appAutomation.messages.lockDeviceFailed") +
          ": " +
          (error.message || t("appAutomation.messages.unknownError")),
      );
    }
...
const unlockDevice = async (device) => {
  try {
    await ElMessageBox.confirm(
      t("appAutomation.messages.unlockDeviceConfirm", {
        name: device.name || device.device_id,
      }),
      t("appAutomation.messages.tip"),
      {
    ...
    if (res.data.success) {
      ...
      ElMessage.error(
        res.data.message || t("appAutomation.messages.unlockDeviceFailed"),
      );
    }
  } catch (error) {
    if (error !== "cancel") {
      ...
      ElMessage.error(
        t("appAutomation.messages.unlockDeviceFailed") +
          ": " +
          (error.message || t("appAutomation.messages.unknownError")),
      );
    }
...
const handleDeleteDevice = async (device) => {
  try {
    await ElMessageBox.confirm(
      t("appAutomation.messages.deleteDeviceConfirm", {
        name: device.name || device.device_id,
      }),
    ...
    if (res.status === 204 || res.status === 200) {
      ...
      ElMessage.error(
        res.data?.message || t("appAutomation.messages.deleteDeviceFailed"),
      );
    }
  } catch (error) {
    if (error !== "cancel") {
      ...
      ElMessage.error(
        t("appAutomation.messages.deleteDeviceFailed") +
          ": " +
          (error.message || t("appAutomation.messages.unknownError")),
      );
    }
...
const getConnectionType = (type) => {
  // emulator、remote_emulator、remote、usb 等类型统一映射。
...
const getConnectionTypeName = (type) => {
  const typeMap = {
    emulator: t("appAutomation.device.connectionTypeMap.emulator"),
    remote_emulator: t("appAutomation.device.connectionTypeMap.remoteEmulator"),
    remote: t("appAutomation.device.connectionTypeMap.remote"),
    usb: t("appAutomation.device.connectionTypeMap.usb"),
  };
