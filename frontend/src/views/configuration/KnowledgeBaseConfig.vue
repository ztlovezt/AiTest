    ...
    <div class="config-content">
      <el-card class="config-card">
        ...
        <el-form :model="form" ref="configForm" label-width="160px" label-position="right">
          ...
          <div class="section-title">
            ...
            {{ $t('configuration.knowledgeBase.zhipuTitle') }}
          </div>
          <el-form-item :label="$t('configuration.aiMode.apiKey')" prop="zhipu_api_key">
            <el-input
              v-model="form.zhipu_api_key"
              type="password"
              :placeholder="currentConfig?.configured ? $t('configuration.aiMode.apiKeyPlaceholderEdit') : $t('configuration.aiMode.apiKeyPlaceholder')"
              show-password
              clearable>
          ...
          <el-form-item :label="$t('configuration.aiMode.baseUrl')" prop="zhipu_base_url">
            <el-input v-model="form.zhipu_base_url" placeholder="默认: https://open.bigmodel.cn/api/paas/v4" clearable>
              <template #prepend><el-icon><Link /></el-icon></template>
            </el-input>
          </el-form-item>
          <el-form-item :label="$t('configuration.knowledgeBase.modelName')" prop="zhipu_vision_model">
            <el-input v-model="form.zhipu_vision_model" placeholder="默认: glm-4v-flash" clearable></el-input>
          </el-form-item>
      ...
      <el-card class="info-card" v-if="currentConfig?.configured">
        ...
        <el-descriptions :column="1" border size="small">
          ...
          <el-descriptions-item label="Zhipu API Key">{{ currentConfig.zhipu_api_key_masked || '未配置' }}</el-descriptions-item>
  ...
  try {
    ...
    if (response.data.configured) {
      form.value = {
        ...
        zhipu_api_key: response.data.zhipu_api_key_masked ? '****' : '',
        zhipu_base_url: response.data.zhipu_base_url || 'https://open.bigmodel.cn/api/paas/v4',
        zhipu_vision_model: response.data.zhipu_vision_model || 'glm-4v-flash'
      }
    } else {
      // 保持默认值
      resetForm()
